...
import com.zgxcw.wx.utils.CustomizedPropertyConfigurer;
...
public class WXController {
  ...
  public void index(HttpServletRequest request, HttpServletResponse response) throws Exception {
    ...
    if (StringUtils.isNotBlank(echostr)) {
      ...
      WXGetwayResponse  wxGetway= wxGetwayService.findById(wxId);
      log.debug("微信网关为:{}", JSON.toJSONString(wxGetway));
      if (null == wxGetway) {
        log.error("根据微信网关id:{}获取网关为空...", wxId);
        return;
      }
      TOKEN=wxGetway.getWxToken();
      //
      ...
      result = sendPassiveMsgService.autoAnswer(xml, wxId);
