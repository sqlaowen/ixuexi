...
//        }
    ...
    public static void trainData() throws Exception {
        String preDic = "D:/opt/yzm";
        File dir = new File(preDic);
        File[] files = dir.listFiles();
        for (File file : files) {
            if(file.isDirectory()){
                continue;
            }
            BufferedImage img = removeBackgroud(file.getAbsolutePath());
            List<BufferedImage> listImg = splitImage(img);
            if (listImg.size() == 2) {
                for (int j = 0; j < listImg.size(); ++j) {
                    ImageIO.write(listImg.get(j), "JPG", new File(preDic+"/train/"
