...
//		itemDto.setExtraCommonParam("{\"id\":\"xxx\"}");
		payItemReqList.add(itemDto);
		 ...
		 itemDto.setExtraCommonParam("{\"id\":\"22222\"}");
		 payItemReqList.add(itemDto);
	...
	public void test02() {
		PayReqDto dto = new PayReqDto();
		dto.setMerchantId("zr001");
		dto.setGatewayCode(GatewayCodeConst.GATEWAY_WXJSAPIPAY);
	...
	public void test04() {
		...
		map.put("discount", "0.00");
		map.put("sign",
				"MCeKYooxJRlVRuEXmogR2ewaKX3XG/Nv9SneXg7nZU/6lWziDKXvrV7Lcay6TE6Iao58ZX3YF4DnPEX5iupMGeDE6x1tF/rSP1vd6kNCdAoU0nyhUWcRPbx7f+aYV/G8L+nK9pO416Yo4y76yiH9hDxXqfkpKPdI+vGhAOGyF/k=");
		map.put("buyer_email", "wenshiwei8888@163.com");
	...
	public void test06() {
		...
		String sign = Signature.getSign(CommonUtil.getMapFromXML(sb.toString() + "</xml>"),
				"CxukqT0OWlXXXrdKFkwfIKXnlFZ11qzC");
	...
	// MergePaymentResDto
	// mergePaymentResDto=mergePaymentService.findById(mergePaymentId);
	// if(null==mergePaymentResDto){
	// System.out.println("===> mergePaymentId:" + mergePaymentId +
	// "未找到合并支付单...");
	// return;
	// }
	// PaymentGatewayResDto
	// paymentGatewayResDto=paymentGatewayService.findById(mergePaymentResDto.getGatewayId());
	// if (null == paymentGatewayResDto) {
	// System.out.println("===> gatewayId:" + mergePaymentResDto.getGatewayId()
	// + "未找到支付网关...");
	// return;
	// }
	//
