	...
	public static Boolean asyncNotify(Map<String, String> map) {
		...
		if (map.get("sign_type").equalsIgnoreCase("MD5")) {
			if (!AlipayNotify.verify(map, map.get("sign_type"), paymentGatewayResDto.getGatewayKey(), paymentGatewayResDto.getVerifyApi(), paymentGatewayResDto.getGatewayAccount())) {
				log.error("ali异步通知(PC)验签不通过,可能遭篡改...,异步通知:{}", map);
				return false;
			}
			...
			if (!AlipayNotify.verify(map, map.get("sign_type"), paymentGatewayResDto.getPublicKey(), paymentGatewayResDto.getVerifyApi(), paymentGatewayResDto.getGatewayAccount())) {
				log.error("ali异步通知(APP)验签不通过,可能遭篡改...,异步通知:{}", map);
				return false;
			}
		}
		...
		if ("TRADE_FINISHED".equalsIgnoreCase(tradeStatus)) { // 退款日期超过可退款期限后（如三个月可退款），支付宝系统发送该交易状态通知;
			// 只做消息记录，不做业务处理
			// ...
		} else if ("WAIT_BUYER_PAY".equalsIgnoreCase(tradeStatus)) { // 移动支付待支付消息推送
			// 只做消息记录，不做业务处理
			// ...
			...
			if(PaymentStateConst.TRADE_CREATE == mergePaymentResDto.getPaymentStateId()){
				...
				ziranNotifyLogicDto.setComments("ali异步处理支付单状态为：支付成功");
				ziranNotifyLogicDto.setBuyerName(createAliNotifyReqDto.getBuyerEmail());
				ziranNotifyLogicDto.setTotalFee(createAliNotifyReqDto.getTotalFee());
				ziranNotifyLogicDto.setPaymentSource(mergePaymentResDto.getPaymentSource());
				ZiranNotifyLogic.notifyLogic(ziranNotifyLogicDto);
			} else {
				log.info("ali异步通知对应合并支付单状态为:{}, 不是交易创建状态, 不进行其它业务处理, 对应异步通知:{}", mergePaymentResDto.getPaymentStateId(), map);
			}
		} else {
			log.info("ali异步通知[ELSE分支]tradeStatus未处理, 对应异步通知:{}", map);
		}
		return true;
	}
