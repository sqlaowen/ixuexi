...
//			ziranNotifyLogicDto.setTradeCloseTime(null);//微信没有这个交易关闭时间返回
			...
			ziranNotifyLogicDto.setBuyerName(createWXNotifyReqDto.getOpenid());
			ziranNotifyLogicDto.setTotalFee(createWXNotifyReqDto.getTotalFee());
			ziranNotifyLogicDto.setPaymentSource(mergePaymentResDto.getPaymentSource());
			
			ZiranNotifyLogic.notifyLogic(ziranNotifyLogicDto);
		} else {
			log.info("wx异步通知对应合并支付单状态为:{}, 不是交易创建状态, 不进行其它业务处理, 对应异步通知:{}", mergePaymentResDto.getPaymentStateId(), map);
		}
		return true;
	}
	...
	private static CreateWXNotifyReqDto bulidWXNotifyReqDto(Map<String, String> map) {
		...
		if (StringUtils.isNotBlank(map.get("total_fee"))) {
			createWXNotifyReqDto.setTotalFee(Long.valueOf(map.get("total_fee")));
		}
