...
//    AlipayImpl.paymentService = paymentService;
//  }
...
//    List<PaymentResDto> paymentResDtoList =paymentService.findByMergePayemntId(resData.getT());
...
//      if (dto.getGatewayCode().equalsIgnoreCase(GatewayCodeConst.GATEWAY_ALIMERGEPAY)) {
//        map = tradeMergePayByUser(paymentResDtoList, gateway, dto.getBankCode());
//        formHtml = AlipaySubmit.getRequestForm(map, gateway.getPayApi(), "post", "MD5", gateway.getGatewayKey());
//      } else {
//        errMsg =String.format("请求平台支付参数[支付网关编码]设置不正确,对应请求:%s...",JSON.toJSONString(dto));
//        log.error(errMsg);
...
//    }
  ...
  private static Map<String, String> createDirectPayByUser(MergePaymentResDto dto,PaymentGatewayResDto gateway) {
    ...
    map.put("subject", "合并支付"); // 商品名
    map.put("payment_type", "1");// 支付类型(1:商品购买)
    map.put("total_fee", String.format("%#.2f", dto.getTotalFee() / 100.0)); // 总金额
    // map.put("extra_common_param", dto.getExtraCommonParam()); // 公用回传参数,此参数不需要传到支付宝,保留在支付单里就可以了
...
//    }
//    sParaTemp.put("merge_trade_info", getMergeTradeInfo(dtoList, gateway)); // 合并支付交易信息
//    return sParaTemp;
//  }
//
//  /**
