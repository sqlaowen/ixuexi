 ...
 */
var TabPanel = function() {}; (function($) {
	/* 组件全局变量 */
	...
	TabPanel = function(id, conf) {
		...
		if (!container.length || !container[0].nodeType) {
			return alert('无法渲染，容器不存在！');
		}

		config = $.extend(dftConfig, conf);

		/* 意外参数处理 */
		if (isNaN(parseInt(config.max))) {
			config.max = dftConfig.max;
		}
		if (!config.tpl || !config.hd || !config.bd) {
			config.tpl = dftConfig.tpl;
		}
		if (isNaN(parseInt(config.owIndex))) {
			config.owIndex = dftConfig.owIndex;
		}
		config.owIndex = Math.min(Math.max(config.owIndex, 0), config.max - 1);

		/* 初始化结构 */
		container.addClass('tabpanel');
		var oHeader = $('<div class="tab-header"><s class="l"></s><s class="r"></s><ul style="margin-top:0px;"></ul></div>').appendTo(container);
		...
		// footer = $('<div class="tabFoot"><s class="l"></s><s
		// class="r"></s></div>').appendTo(el);
		...
		$(window).resize(function() {
			...
			//console.log('其它height:'+otherHeight);
			//console.log('容器height:'+container.height());
	...
	TabPanel.prototype = {
		...
		activeTab: function(conf, reload) { // 直接显示某个tab
			...
			if ($.isFunction(config.onActive)) {
				config.onActive.apply(this, [id, hd, bd]);
			}
			return this;
		},
		/**
		...
		addTab: function(conf) { // 创建tab
			...
			var closeicon = showClose ? "<s class='close'>关闭</s>": '';

			var data = $.extend(conf, {
				id: id,
				icon: icon,
				closeicon: closeicon,
				closeCls: showClose ? 'closeCls': ''
			});
			var hd, bd;
			...
			if ($.isFunction(config.onAdd)) {
				config.onAdd.apply(this, [id, hd, bd]);
			}
			return this;
		},
		/**
		...
		removeTab: function(index, autoActive) {
			...
			tabCache.body.splice(index, 1); --tabCount;

			try {
				CollectGarbage();
			} catch(e) {}
			if ($.isFunction(config.onRemove)) {
				config.onRemove.apply(this, [hd.attr('id'), hd, bd]);
			}
			// 自动激活相邻的Tab
			if (autoActive) {
		...
		getId: function(conf) { // 根据url获取tab的id
			...
			if (!conf || !conf.url || typeof conf == 'string') {
				return conf || '';
			}
		...
		fixIFrame: function(iframe) {
			...
			if (iframe.get(0).tagName.toLowerCase() != 'iframe') {
...
(function($) {
	 组件全局变量 
	var container, config = {}, idCache = {}, tabCount = 0, tabCache = {
		header : [],
		body : []
	};
	var header, body, footer;
	...
	var toInt = function(o) {
	 ...
	 获取上下边框及margin总和 
	var getPx = function(o) {
		return toInt(o.css('margin-top')) + toInt(o.css('margin-bottom'))
				+ toInt(o.css('border-top-width'))
				+ toInt(o.css('border-bottom-width'));
	};
	 获取左右边框及margin总和 
	var getHpx = function(o) {
		return toInt(o.css('margin-left')) + toInt(o.css('margin-right'))
				+ toInt(o.css('border-right-width'))
				+ toInt(o.css('border-left-width'));
	};
	 获取当前文件路径 
	var scripts = document.getElementsByTagName('script');
	var curScript = scripts[scripts.length - 1];
	...
	TabPanel.prototype = {
		...
		*//**
		...
		addTab : function(conf) { // 创建tab
			...
			icon = isIE6 ? "<img src='"
					+ config.blankImg
					+ "' style=\"filter: progid:DXImageTransform.Microsoft.AlphaImageLoader(sizingMethod='image', src='"
					+ icon + "')\" />"
					: "<img src='" + icon + "' />";
			...
			header.attr('iw', parseInt(header.attr('iw'))
					+ parseInt(hd.attr('w', hd.outerWidth() + getHpx(hd)).attr('w'))); // 增加内容总宽
			calcTab.call(this);
