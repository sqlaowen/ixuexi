...
public class MainApp2 {
    private Logger log= LoggerFactory.getLogger(MainApp2.class);
    public static void main(String[] args) {

        String preUrl = "http://www.144cao.com";
        String urlFmt = "http://www.144cao.com/html/part/index37%s.html";
        for (int i = 0; i < 49; i++) {
            String url = "";
            if (i == 0) {
...
//                    System.out.println("\r\n\r\n");
    ...
    static String getHtml(String url) {
        String html = HttpReqUtil.httpGetRequest(url, "GBK");
        if (StringUtils.isBlank(html)) {
            for (int j = 0; j < 3; j++) {
                try {
                    Thread.sleep(200);
