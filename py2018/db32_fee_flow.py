...
f = open('d:/xstore_binvoice_fee_flow.txt', 'w')
strFmt = '%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s'

url = 'http://dbquery.jd.com/home/ajaxQueryData'

# 开票费用台账表
sqlFmt = 'select id,account_id,fee_code,uuid,amount,direction,created,modified,remark,flow_status,rf_bill_type,rf_bill_no,bill_id  from xstore_binvoice_fee_flow%s where id>%s order by id asc'

# 开票费用台账表
sqlCountFmt = 'select count(1) from xstore_binvoice_fee_flow%s'
            ...
            try:
                ...
                for x in xlist:
                    if x == 'rows':
                        for r in xlist[x]:
                            f.write(strFmt % (
                            r['id'], r['account_id'], r['fee_code'], r['uuid'], r['amount'], r['direction'],
                            r['created'], r['modified'], r['remark'], r['flow_status'], r['rf_bill_type'],
                            r['rf_bill_no'], r['bill_id']))
