...
f = open('d:/xstore_bill_detail.txt', 'w');

strFmt = '%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s'

url = 'http://dbquery.jd.com/home/ajaxQueryData'

# 业务明细表
sqlFmt = 'select id,rf_bill_type,rf_bill_no,rf_bill_detail_no,uuid,order_id,order_type,rf_busi_id,rf_busi_type,fee_code,fee_name,invoice_method,direction,currency,bal,account_id,status,bill_type,bill_id,created,related_time,audited,modified,remark from xstore_bill_detail%s where invoice_method=20 and id>%s order by id asc'
        ...
        for j in range(0, page):
            ...
            try:
                ...
                for x in xlist:
                    if x == 'rows':
                        for r in xlist[x]:
                            f.write(strFmt % (r['id'],r['rf_bill_type'],r['rf_bill_no'],r['rf_bill_detail_no'],r['uuid'],r['order_id'],r['order_type'],r['rf_busi_id'],r['rf_busi_type'],r['fee_code'],r['fee_name'],r['invoice_method'],r['direction'],r['currency'],r['bal'],r['account_id'],r['status'],r['bill_type'],r['bill_id'],r['created'],r['related_time'],r['audited'],r['modified'],r['remark']))
